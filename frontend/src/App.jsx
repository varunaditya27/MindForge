...
import Chatbot from "./components/chatbot"; // file name lowercase, export MUST be Chatbot
...
function App() {
  ...
  const handleSignOut = () => {
    const userKey = user?.email || user?.uid;
    if (userKey) clearUserData(userKey);
    setUser(null);
    setUserProfile(null);
    setFeedback(null);
    setScores(null);
  ...
  const MainPage = () => {
    ...
    if (!userProfile?.branch || !userProfile?.rollNumber) {
      return (
        <>
          <Particles />
          <UserProfileSetup user={user} onProfileComplete={handleProfileComplete} />
        </>
      );
    }
          ...
          <main id="main" role="main" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid grid-cols-1 gap-8">
              {!hasSubmittedThisRound ? (
                <IdeaSubmissionForm userProfile={userProfile} onSubmissionSuccess={handleSubmissionSuccess} />
              ) : (
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        {scores?.totalScore != null && (
                          <span className="text-amber-200 text-sm">
                            Last score: {scores.totalScore}/100
                          </span>
                        )}
                      </div>
                    </div>
            ...
            <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
              <div className="flex flex-col items-center">
                ...
                <div className="flex flex-wrap items-center justify-center gap-4 text-[11px] tracking-wide uppercase text-[#a87454] mb-4">
                  <span className="px-3 py-1 rounded-full bg-[#1d120c]/70 border border-[#3a2516]">
                    Forged at RVCE
                  </span>
                  <span className="px-3 py-1 rounded-full bg-[#1d120c]/70 border border-[#3a2516]">
                    AI Tempered
